...
problem: 'The website distributes an Android APK whose backend API is vulnerable to command injection. Reaching the injectable endpoint requires emulator setup with proxy interception. Privilege escalation relies on a vulnerable sudo version (CVE-2021-3156).'
action: 'Nmap found SSH (22) and HTTP (80). Downloaded the APK, extracted routerspace.htb from the signer certificate via MobSF, installed it on a Genymotion emulator, and configured BurpSuite proxy interception via ADB reverse port forwarding. Intercepted a POST to /api/v4/monitoring/router/dev/check/deviceAccess and found command injection in the ip parameter. Injected an SSH public key into paul''s authorized_keys for a stable shell. Identified sudo 1.8.31 as vulnerable to CVE-2021-3156 and ran the exploit for root.'
outcome: 'Rooted the box. APK analysis led to API command injection as paul, then CVE-2021-3156 sudo exploit for root.'
draft: false
---
