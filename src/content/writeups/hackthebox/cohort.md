...
problem: 'Linux host behind nginx with a URL validation endpoint vulnerable to SSRF (loopback filter bypassable via 127.1). Internal marimo notebook on port 8888 has a pre-auth WebSocket RCE (CVE-2026-39987). PackageKit 1.2.8 on D-Bus is vulnerable to a TOCTOU race (CVE-2026-41651) for root.'
action: 'RustScan and nmap found SSH (22), HTTP (80), HTTPS (443) with nginx 1.24.0 and wildcard SAN *.cohort.htb. Found /portal.html with a URL validation form POSTing to /api/validate. Bypassed loopback filter with 127.1 for SSRF, scanned internal ports — found marimo notebook on 8888. SSRF on /status leaked the hidden vhost nb-1be3782a8afd3ad5.cohort.htb. Wrote a custom Python exploit for CVE-2026-39987 (marimo /terminal/ws WebSocket missing auth) with raw TLS and WebSocket handshake, got shell as marimo. Ran CVE-2026-41651 exploit (PackageKit TOCTOU race) which dropped a setuid root bash at /tmp/.suid_bash for root access.'
outcome: 'Gained root through SSRF to internal service discovery, marimo WebSocket pre-auth RCE (CVE-2026-39987) for foothold, and PackageKit TOCTOU race (CVE-2026-41651) for privilege escalation.'
draft: false
---
