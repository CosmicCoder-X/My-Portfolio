...
problem: 'Ubuntu machine with SSH and HTTP. Directory enumeration reveals OpenNetAdmin 18.1.1 with a known unauthenticated RCE. Database credentials are stored in plaintext and reused by jimmy, an internal web app on port 52846 exposes joanna''s crackable SSH key, and joanna has passwordless sudo access to nano.'
action: 'Nmap found SSH (22) and HTTP (80) with Apache 2.4.29. Dirsearch discovered /music redirecting to /ona/ running OpenNetAdmin 18.1.1. Exploited the known RCE for a shell as www-data. Found database credentials (n1nj4W4rri0R!) in /opt/ona/www/local/config/database_settings.inc.php and used the password to su to jimmy. Discovered an internal web app on port 52846 via netstat; curled localhost:52846/main.php to retrieve joanna''s encrypted SSH key. Cracked the passphrase (bloodninjas) with ssh2john and john, then SSHed in as joanna. Sudo -l revealed NOPASSWD nano on /opt/priv; used nano''s execute command feature (Ctrl+R, Ctrl+X) to spawn a root shell.'
outcome: 'Root access via four-stage chain: OpenNetAdmin RCE to www-data, password reuse to jimmy, cracked SSH key to joanna, and sudo nano GTFOBins to root.'
draft: false
---
