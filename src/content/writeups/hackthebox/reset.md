...
problem: 'The password reset endpoint returns the new plaintext password in its JSON response. The admin dashboard has an unrestricted LFI in the log-viewer file parameter, and Apache logs User-Agent headers verbatim enabling log poisoning. A hosts.equiv file grants passwordless rlogin as sadm, who has a sudo rule for nano.'
action: 'Nmap found SSH (22), Apache (80) with an Admin Login page, and r-services (512-514). POSTed username=admin to reset_password.php and received the new password in plaintext JSON. Authenticated as admin, found an LFI in the dashboard log-viewer, and confirmed it reads arbitrary files including Apache access.log. Poisoned the log with a PHP reverse shell in the User-Agent header, triggered via LFI, and caught a shell as www-data. Exploited a hosts.equiv trust entry to rlogin as sadm without a password, recovered sadm''s sudo password from a running tmux session, then used the GTFOBins nano sudo escape (Ctrl+R, Ctrl+X, shell command) for root.'
outcome: 'Gained root access. The chain was password reset disclosure to admin, LFI plus log poisoning to www-data, rlogin trust abuse to sadm, and nano sudo escape to root.'
draft: false
---
