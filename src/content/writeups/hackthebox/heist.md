...
problem: 'Windows Server 2019 with HTTP (80), MSRPC (135), and SMB (445). A guest-accessible support portal exposes a Cisco config with crackable password hashes. RID brute-forcing reveals domain users, password spraying grants WinRM access as Chase, and Firefox process memory contains cached admin credentials leading to SYSTEM via psexec.'
action: 'Nmap found HTTP (80), MSRPC (135), and SMB (445). Guest login on /issues.php revealed a Cisco config with type 5 and type 7 hashes — cracked the type 5 with john (stealth1agent) and decoded the type 7 passwords. RID brute-forced with crackmapexec using Hazard:stealth1agent to enumerate domain users. Password sprayed to get evil-winrm as Chase. Dumped the running Firefox process with procdump, grepped for login_password, and recovered admin credentials (4dD!5}x/re8]FBuZ). Used psexec.py as administrator for SYSTEM.'
outcome: 'SYSTEM via a credential recovery chain — Cisco config hashes to initial creds, RID brute-force and password spray for WinRM as Chase, Firefox memory dump for admin credentials, and psexec for SYSTEM.'
draft: false
---
