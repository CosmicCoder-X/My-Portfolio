...
difficulty: 'medium'
date: 2025-08-29
summary: 'An AI/ML challenge — given two NumPy binary files (embeddings.npy containing high-dimensional vectors and tokens.npy containing individual characters), using PCA to reduce the embedding dimensions to 3, plotting the result with 2D projections onto the XY, XZ, and YZ planes, noticing an unnaturally clean spiral pattern in the XY plane shadow, adding token labels to the projections, and reading the flag from the characters arranged along the spiral.'
role: 'llm'
tags: ['ai', 'machine-learning', 'embeddings', 'pca', 'dimensionality-reduction', 'numpy', 'matplotlib', 'scikit-learn', 'nlp', 'tokenization']
problem: 'Two .npy files are provided — embeddings.npy (a matrix of high-dimensional vectors) and tokens.npy (an array of individual characters). The challenge description hints at shadows and dimensions: "A cube is the shadow of a tesseract casted on 3 dimensions. I wonder what other secrets may the shadows hold." The flag is hidden somewhere in the relationship between the embeddings and their lower-dimensional projections.'
action: 'Loaded both .npy files in Google Colab with NumPy. The embeddings array contained high-dimensional floating-point vectors and the tokens array contained individual characters including letters, digits, and symbols. Used scikit-learn PCA to reduce the embedding vectors from their original dimensionality down to 3 principal components, then plotted the reduced vectors in a 3D scatter plot with 2D shadow projections onto the XY, XZ, and YZ planes. The XY plane projection showed an unnaturally clean spiral pattern that stood out against the otherwise scattered points. Added token character labels to each point in the projections, which revealed the flag spelled out along the spiral path in the XY shadow.'
