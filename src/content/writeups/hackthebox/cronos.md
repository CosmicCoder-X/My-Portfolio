...
problem: 'A Linux machine running SSH, DNS BIND 9.10.3, and Apache. Browsing the IP shows only the default page — the real application is hidden behind DNS virtual hosts, with an SQLi-vulnerable admin panel, command injection on a network tool, and a crontab misconfiguration for privesc.'
action: 'Nmap revealed SSH (22), DNS BIND 9.10.3 (53), and Apache (80). DNS zone transfer on cronos.htb disclosed admin.cronos.htb. Bypassed the admin login with SQLi (admin'' #), confirmed via sqlmap. Exploited command injection on Net Tool v0.1 — bash reverse shell failed, pivoted to Python reverse shell for a www-data shell. Found a crontab running php /var/www/laravel/artisan as root every minute with the directory owned by www-data — replaced artisan with a PHP reverse shell for root.'
outcome: 'Gained root access. Attack chain: DNS zone transfer, SQLi auth bypass, command injection for www-data shell, crontab abuse replacing the root-executed artisan file for privesc.'
draft: false
---
