...
problem: 'Windows Server with anonymous FTP exposing the entire C:\ drive, PRTG Network Monitor 18.1.37.13946 on port 80, and standard Windows services. FTP leaks PRTG config backups with an old password PrTg@dmin2018, and the PRTG notification system has an authenticated RCE via command injection in the Demo exe script parameters.'
action: 'Nmap identified FTP (21, anonymous), HTTP (80, PRTG 18.1.37.13946), MSRPC, NetBIOS, and SMB. Retrieved user.txt via anonymous FTP from C:\Users\Public. Located PRTG config backups at C:\ProgramData\Paessler\PRTG Network Monitor\ via FTP; the .old.bak file contained PrTg@dmin2018 in plaintext. Incremented year to PrTg@dmin2019 and logged in as prtgadmin. Created a notification with the Demo exe script, injected a command to fetch nc.exe from an attacker SMB share and spawn a reverse shell. Received a shell as NT AUTHORITY\SYSTEM.'
outcome: 'SYSTEM access via anonymous FTP credential recovery and authenticated PRTG RCE. FTP exposed the filesystem and config backups, year-incremented password granted PRTG admin, and notification command injection delivered a SYSTEM shell.'
draft: false
---
