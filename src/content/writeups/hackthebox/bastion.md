...
problem: 'Windows Server 2016 with a guest-accessible Backups SMB share containing a full WindowsImageBackup with VHD files. Mounting the VHD exposes SAM/SYSTEM hives for offline hash extraction. L4mpje''s cracked credentials grant SSH, and mRemoteNG stores the decryptable Administrator password in its config.'
action: 'Ran AutoRecon — identified SSH (22), SMB (445), WinRM (5985), and RPC services on Windows Server 2016. Smbmap revealed the Backups share with guest READ/WRITE access. Mounted the share, found a WindowsImageBackup with two VHD files, mounted the larger one with guestmount. Copied SAM and SYSTEM hives from Windows/System32/config, dumped NTLM hashes with samdump2 and impacket-secretsdump. Cracked L4mpje''s hash to bureaulampje via CrackStation, SSHed in (evil-winrm failed). Ran linpeas, discovered mRemoteNG with stored credentials, decrypted the Administrator password from confCons.xml, and SSHed in as Administrator.'
outcome: 'Gained Administrator through offline credential extraction from a VHD backup (SAM/SYSTEM hive dumping) and mRemoteNG stored credential decryption. No exploits or CVEs — just exposed backups and stored passwords.'
draft: false
---
