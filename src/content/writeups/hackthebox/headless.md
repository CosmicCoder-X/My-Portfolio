...
problem: 'A Python web app on port 5000 with a /support form and admin-only /dashboard. The is_admin cookie lacks HttpOnly, and a headless browser renders submitted reports including request headers — creating an XSS vector via User-Agent that can exfiltrate the admin cookie.'
action: 'Found /support form where angle brackets trigger a "Hacking Attempt Detected" page reflecting full request headers. OWASP ZAP confirmed the is_admin cookie lacks HttpOnly and User-Agent XSS payloads are reflected. Injected an XSS payload in User-Agent to exfiltrate document.cookie when the headless browser rendered the report, catching the admin cookie on a listener. Used the stolen cookie to access /dashboard, exploited command injection in the date parameter for a reverse shell as dvir, then escalated to root via sudo on a syscheck script that executes a user-controlled initdb.sh.'
outcome: 'Rooted via XSS cookie theft through User-Agent reflection, command injection on the admin dashboard for a shell as dvir, and sudo abuse of syscheck/initdb.sh for root.'
draft: false
---
