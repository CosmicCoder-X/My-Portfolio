...
difficulty: 'easy'
date: 2025-01-13
summary: 'An HTB machine — scanning with rustscan and nmap to find 14 open ports on a Windows domain controller including DNS (53), HTTP (80) running IIS 10.0 serving "Egotistical Bank :: Home", Kerberos (88), LDAP (389/3268) for the EGOTISTICAL-BANK.LOCAL domain, SMB (445), WinRM (5985), and others, attempting zone transfers on DNS which fail, testing rpcclient and smbclient with null/anonymous sessions which are rejected, querying LDAP with ldapsearch to confirm the domain structure, brute-forcing Kerberos usernames with kerbrute against xato-net-10-million-usernames.txt to discover hsmith, Administrator, and fsmith, AS-REP roasting with impacket-GetNPUsers to obtain fsmith''s hash (UF_DONT_REQUIRE_PREAUTH set), cracking the hash with hashcat mode 18200 to reveal Thestrokes23, logging in via evil-winrm as FSmith to retrieve the user flag, uploading winPEAS via impacket-smbserver to discover AutoLogon credentials for svc_loanmanager with password Moneymakestheworldgoround!, running BloodHound to identify DCSync permissions on svc_loanmgr, dumping all domain hashes with impacket-secretsdump using the svc_loanmgr credentials, and using impacket-psexec with the Administrator NTLM hash for a pass-the-hash attack to gain a SYSTEM shell.'
role: 'pentest'
...
problem: 'Sauna is an easy-rated Windows domain controller for the EGOTISTICAL-BANK.LOCAL domain with numerous open ports including DNS (53), HTTP (80) running IIS 10.0, Kerberos (88), LDAP (389/3268), SMB (445), WinRM (5985), and .NET Message Framing (9389). The domain has a user fsmith with Kerberos pre-authentication disabled, making the account vulnerable to AS-REP roasting. The machine stores AutoLogon credentials in the registry for the svc_loanmanager service account with the password Moneymakestheworldgoround!. The svc_loanmgr account has DCSync privileges (DS-Replication-Get-Changes and DS-Replication-Get-Changes-All), allowing it to replicate the domain''s NTDS.DIT and extract all user hashes including the Administrator account.'
action: 'Ran rustscan to quickly identify 14 open ports, then nmap with default scripts (-sC) and version detection (-sV) to enumerate services — DNS (53) running Simple DNS Plus, HTTP (80) running IIS 10.0 with title "Egotistical Bank :: Home", Kerberos (88), MSRPC (135), NetBIOS (139), LDAP (389/3268) for domain EGOTISTICAL-BANK.LOCAL, SMB (445), Kpasswd (464), NCACN_HTTP (593), tcpwrapped (636/3269), WinRM (5985), and .NET Message Framing (9389). Host identified as SAUNA running Windows with a 7-hour clock skew and SMB signing required. Attempted DNS zone transfers against egotistical-bank.local and sauna.htb — both failed. Tested rpcclient with a null username and smbclient with both default and anonymous credentials — all rejected with NT_STATUS_LOGON_FAILURE. Queried LDAP with ldapsearch using anonymous bind to confirm the domain naming contexts (DC=EGOTISTICAL-BANK,DC=LOCAL) and enumerate domain objects, though user enumeration via LDAP was limited. Used kerbrute to brute-force valid usernames against Kerberos with xato-net-10-million-usernames.txt — discovered hsmith, Administrator, and fsmith. Ran impacket-GetNPUsers with the discovered usernames to perform AS-REP roasting — hsmith did not have UF_DONT_REQUIRE_PREAUTH set, but fsmith returned a Kerberos AS-REP hash. Identified the hashcat mode as 18200 and cracked the hash with rockyou.txt — revealing Thestrokes23 as fsmith''s password. Logged in via evil-winrm as FSmith and retrieved the user flag. Uploaded winPEAS via impacket-smbserver and executed it — discovered AutoLogon credentials in the registry with DefaultDomainName EGOTISTICALBANK, DefaultUserName EGOTISTICALBANK\svc_loanmanager, and DefaultPassword Moneymakestheworldgoround!. Ran BloodHound and identified that svc_loanmgr has DCSync permissions (Outbound Object Control). Used impacket-secretsdump with svc_loanmgr:Moneymakestheworldgoround! to dump all domain hashes via DRSUAPI — obtained NTLM hashes for Administrator, Guest, krbtgt, HSmith, FSmith, svc_loanmgr, and SAUNA$. Verified the Administrator hash with crackmapexec (Pwn3d!). Used impacket-psexec with the Administrator NTLM hash to perform a pass-the-hash attack — obtained a SYSTEM shell on the domain controller.'
outcome: 'Gained SYSTEM-level access on the domain controller through a multi-stage Active Directory attack. The attack chain was rustscan and nmap enumeration identifying the EGOTISTICAL-BANK.LOCAL domain, kerbrute username enumeration via Kerberos, AS-REP roasting fsmith''s hash due to disabled pre-authentication, hashcat cracking the password to Thestrokes23, evil-winrm login as FSmith, winPEAS discovery of svc_loanmanager AutoLogon credentials, BloodHound identification of DCSync privileges on svc_loanmgr, impacket-secretsdump to dump all domain NTLM hashes, and impacket-psexec pass-the-hash as Administrator for SYSTEM access.'
draft: false
---
