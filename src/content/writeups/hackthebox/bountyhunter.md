...
problem: 'Ubuntu host with SSH (22) and Apache (80). A bounty report form processes XML without disabling external entities (XXE). Database credentials in db.php are reused for SSH. A sudo NOPASSWD Python ticket validator uses eval() on user-supplied Markdown ticket data.'
action: 'Masscan and nmap found SSH (22) and HTTP (80). Dirb discovered db.php, portal.php, and log_submit.php. Intercepted the bounty form submission in Burp — base64-encoded XML sent to /tracker_diRbPr00f314.php. Injected XXE payloads to read /etc/passwd, then used php://filter/convert.base64-encode to exfiltrate db.php containing credentials (password m19RoAU0hP41A1sTsq6K). SSHed in as development with the database password. Found sudo NOPASSWD on ticketValidator.py which uses eval() on ticket code lines — crafted a malicious .md ticket with __import__("os").system() to spawn a root reverse shell.'
outcome: 'Gained root through XXE credential exfiltration for SSH access and Python eval() injection in a sudo-permitted script for privilege escalation.'
draft: false
---
