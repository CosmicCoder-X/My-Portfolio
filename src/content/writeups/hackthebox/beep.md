...
difficulty: 'easy'
date: 2025-09-15
summary: 'An HTB machine — scanning with nmap to find a heavily-serviced CentOS box running SSH (22), SMTP (25), Apache (80/443) with Elastix, POP3/IMAP (110/143/993/995), RPC (111), MySQL (3306), and Webmin on MiniServ 1.570 (10000), fixing a Firefox TLS compatibility issue to access the HTTPS Elastix login page, running gobuster to discover directories including /admin (revealing FreePBX 2.8.1.4), finding CVE-2012-4869 (FreePBX/Elastix pre-authenticated RCE via the callme_page.php recordings module), enumerating SIP on port 5060 with sippts to fingerprint Asterisk PBX and using svwar to discover extension 233, running the exploit with a reverse shell payload to catch a shell as the asterisk user, discovering extensive passwordless sudo privileges including nmap, then escalating to root via nmap''s interactive mode (!sh shell escape).'
role: 'pentest'
...
problem: 'Beep is an easy-rated Linux (CentOS) machine running a wide array of services — SSH (22), SMTP (25), HTTP/HTTPS (80/443) hosting Elastix with FreePBX 2.8.1.4, POP3 (110), RPC (111), IMAP (143), IMAPS (993), POP3S (995), MySQL (3306), an unidentified service on 4445, and Webmin/MiniServ 1.570 on port 10000. The HTTPS site requires legacy TLS support to access. The Elastix/FreePBX installation is vulnerable to CVE-2012-4869, a pre-authenticated remote code execution via the callme_page.php recordings module that requires a valid SIP extension number. The asterisk service user has extensive passwordless sudo privileges including nmap, which has a known interactive mode shell escape.'
action: 'Ran nmap with service version detection — identified 12 open ports including SSH (22), SMTP (25), HTTP (80), POP3 (110), RPC (111), IMAP (143), HTTPS (443) running Apache 2.2.3 on CentOS with an Elastix login page, IMAPS (993), POP3S (995), MySQL (3306), upnotify (4445), and Webmin/MiniServ 1.570 on port 10000. Firefox refused to connect to HTTPS due to the server''s outdated TLS — resolved by setting security.tls.version.min to 1 in about:config. The Elastix login page loaded but default credentials failed. The Webmin login on port 10000 also rejected attempts. Ran gobuster against the HTTPS site with the dirb/big.txt wordlist and the -k flag to skip certificate validation — found directories including /admin (revealing FreePBX 2.8.1.4), /configs, /help, /images, /lang, /libs, /mail, /modules, /panel, /recordings, and /static. Researched CVE-2012-4869 on Exploit-DB — a FreePBX/Elastix pre-authenticated RCE (exploit 18650) targeting the callme_page.php recordings module, requiring a valid SIP extension number. Enumerated SIP on port 5060 with sippts — fingerprinted the service as Asterisk PBX using the REGISTER method response (FPBX-2.8.1(1.8.7.0) User-Agent). Used svwar to brute-force extensions 100-500 with the INVITE method — discovered extension 233 with reqauth authentication. Modified the exploit script with the correct extension (233), attacker IP, and port. First attempted to run with python3 — failed due to urllib attribute error. Ran with python2 successfully — caught a reverse shell on the netcat listener from the target as the asterisk user. Ran sudo -l — the asterisk user could execute 14 commands as root without a password: /sbin/shutdown, /usr/bin/nmap, /usr/bin/yum, /bin/touch, /bin/chmod, /bin/chown, /sbin/service, /sbin/init, /usr/sbin/postmap, /usr/sbin/postfix, /usr/sbin/saslpasswd2, /usr/sbin/hardware_detector, /sbin/chkconfig, and /usr/sbin/elastix-helper. Used sudo nmap --interactive to enter nmap''s interactive mode (Nmap 4.11), then !sh to spawn a root shell. Retrieved both flags.'
outcome: 'Gained root access to the machine. The attack chain was nmap service enumeration across 12 ports, TLS compatibility fix for HTTPS access, gobuster directory discovery revealing FreePBX 2.8.1.4, SIP extension enumeration with sippts and svwar (extension 233), CVE-2012-4869 FreePBX/Elastix pre-authenticated RCE for a shell as asterisk, and nmap interactive mode sudo escape for root.'
draft: false
---
