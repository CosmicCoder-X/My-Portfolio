...
difficulty: 'easy'
date: 2025-08-29
summary: 'An HTB machine — enumerating a Ruby on Rails website running Camaleon CMS v2.9.0 behind nginx on facts.htb, discovering /admin with gobuster and registering an account, exploiting CVE-2024-46987 (Camaleon CMS file read) to read /etc/passwd and enumerate users trivia and william, exploiting a mass assignment vulnerability to escalate the registered user to Admin role, extracting AWS S3 keys from the Filesystem Settings pointing to a MinIO instance on port 54321, using AWS CLI to dump an internal bucket containing an SSH private key, cracking the key passphrase with John the Ripper (dragonballz), SSH-ing in as trivia, then escalating to root via sudo /usr/bin/facter — a Ruby script with no input validation — by loading a malicious custom fact from an attacker-controlled directory.'
role: 'pentest'
...
problem: 'Facts is an easy-rated Linux machine running SSH (22), nginx (80) hosting a Ruby on Rails application with Camaleon CMS v2.9.0, and MinIO (54321) serving as an S3-compatible object store. The CMS has publicly known vulnerabilities including a file read and a mass assignment flaw, the admin panel exposes AWS S3 credentials, and an internal S3 bucket contains an SSH private key. Privilege escalation is through a sudo-allowed Facter script that loads arbitrary Ruby code from user-specified directories.'
action: 'Ran nmap to identify open ports — 22/tcp (OpenSSH 9.9p1), 80/tcp (nginx 1.26.3), and 54321/tcp (MinIO/Golang redirecting to port 9001). Added facts.htb to /etc/hosts. Browsed the website — a trivia application called Facts powered by Ruby on Rails with Bootstrap 3.4.1, jQuery 2.2.4, and Modernizr 2.6.2 as identified by Wappalyzer. Ran gobuster and discovered /admin endpoints (admin, admin.cgi, admin.php, admin.pl all returning status 200 with size 3896). Browsed to /admin/login and found a Camaleon CMS login page with registration enabled. Created an account (nooff) and logged in to the admin dashboard. Identified Camaleon CMS v2.9.0 from the page footer. Found CVE-2024-46987 — a file read vulnerability in Camaleon CMS. Ran the PoC exploit to read /etc/passwd, enumerating two users with bash shells — trivia (uid=1000) and william (uid=1001). Exploited a mass assignment vulnerability in the profile update endpoint — intercepted the POST to /admin/users/5/updated_ajax and added role=admin to the password update parameters, escalating the nooff user to Administrator. With admin access, navigated to Settings > Filesystem Settings and found AWS S3 credentials — access key AKIA08F009B89E5C8894, secret key dpHtrcl7Cfzd7Nm48XlhFu+xQExHXOM9ntzelAY1, bucket name randomfacts, region us-east-1, endpoint http://localhost:54321, and CloudFront URL http://facts.htb/randomfacts. Confirmed S3 involvement from X-Amz-Id-2 and X-Amz-Request-Id headers in HTTP responses for image assets. Configured AWS CLI with the discovered keys and enumerated buckets via the MinIO endpoint — found two buckets (internal, randomfacts). Listed the internal bucket contents and found a .ssh directory. Dumped the entire internal bucket recursively with aws s3 cp, discovering an ed25519 SSH private key. The key was passphrase-protected — converted it with ssh2john and cracked it with John the Ripper using rockyou.txt, recovering the passphrase dragonballz. Used the key to SSH in as trivia. Read the user flag from /home/william/user.txt. Checked sudo -l and found (ALL) NOPASSWD: /usr/bin/facter. Examined the script — a Ruby wrapper for the Facter CLI tool that passes ARGV directly to CliLauncher without input validation. Created a malicious Ruby fact (/tmp/exploit.rb) defining a custom fact named anything that executes /bin/bash -p. Ran sudo /usr/bin/facter --custom-dir /tmp anything to load the malicious fact and spawn a root shell.'
outcome: 'Gained root access to the machine. The attack chain was Camaleon CMS file read (CVE-2024-46987) for user enumeration, mass assignment for admin privilege escalation, AWS S3 credential extraction and bucket dumping for SSH key recovery, passphrase cracking for initial SSH access as trivia, and Facter custom fact injection via sudo for root.'
draft: false
---
