...
problem: 'Three open ports — SSH (22), HTTP (80) serving a single image, and Elasticsearch (9200) unauthenticated. The image contains a base64 hint pointing to credentials in Elasticsearch, Kibana on localhost is vulnerable to CVE-2018-17246 (LFI), and Logstash runs as root executing commands from watched log files writable by the kibana group.'
action: 'Nmap found SSH (22), HTTP (80), and Elasticsearch (9200). Extracted a base64 hint from needle.jpg via strings, decoded it to "la aguja en el pajar es clave," then searched the Elasticsearch quotes index for "clave" to find base64-encoded credentials for security:spanish.is.key. SSHed in, discovered Kibana on localhost:5601 via /proc/net/tcp, and exploited CVE-2018-17246 (Kibana LFI) to execute a Node.js reverse shell as kibana. Read Logstash configs in /etc/logstash/conf.d/ showing a pipeline that watches /opt/kibana/logstash_* and executes commands matching a Grok pattern. Wrote a crafted entry with a bash reverse shell to /opt/kibana/logstash_0xdf to get root.'
outcome: 'Rooted via three Elastic Stack stages — steganographic hint to Elasticsearch credentials, Kibana LFI for lateral movement to kibana, and Logstash command execution from a crafted log entry for root.'
draft: false
---
