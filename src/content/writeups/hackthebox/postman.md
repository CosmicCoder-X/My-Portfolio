...
problem: 'Ubuntu 18.04 with SSH, HTTP, unauthenticated Redis 4.0.9 (running as the redis user with a valid shell at /var/lib/redis), and Webmin 1.910. Redis allows filesystem writes for SSH key injection, /opt/id_rsa.bak contains Matt''s crackable encrypted key, and Matt reuses the passphrase for Webmin which is vulnerable to CVE-2019-12840.'
action: 'Naabu and nmap identified SSH (22), HTTP (80), Redis (6379, no auth), and Webmin (10000). Connected to Redis, confirmed no requirepass, generated an SSH key pair with newline padding, injected it via redis-cli -x set ssh_key, set dir to /var/lib/redis/.ssh and dbfilename to authorized_keys, saved, and SSHed in as redis. Linpeas found /opt/id_rsa.bak; cracked the passphrase (computer2008) with ssh2john and john. SSH as Matt was rejected but su Matt from the redis shell worked. Logged into Webmin with Matt:computer2008 and exploited CVE-2019-12840 via Metasploit for root.'
