...
problem: 'Linux machine with SSH and nginx redirecting to orion.htb running Craft CMS 5.6.16. The CMS is vulnerable to pre-auth RCE (CVE-2025-32432), the .env file contains database credentials reused by a local user, and a locally-bound telnetd has an auth bypass vulnerability.'
action: 'Nmap found SSH (22) and nginx (80) redirecting to orion.htb. Gobuster discovered /admin/login identifying Craft CMS 5.6.16. Exploited CVE-2025-32432 via Metasploit for a Meterpreter session as www-data. Extracted DB credentials from the .env file (CRAFT_DB_PASSWORD=SuperSecureCraft123Pass!), queried MariaDB for adam''s bcrypt hash, and cracked it with hashcat to darkangel. SSHed in as adam, found telnet (GNU inetutils 2.7) on localhost:23, and exploited CVE-2026-24061 auth bypass with env USER="-f root" telnet -a 127.0.0.1 23 for root.'
outcome: 'Root access via three-stage chain: Craft CMS pre-auth RCE (CVE-2025-32432) to www-data, cracked DB hash for SSH as adam, and telnetd auth bypass (CVE-2026-24061) to root.'
draft: false
---
