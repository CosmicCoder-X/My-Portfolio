...
difficulty: 'easy'
date: 2025-09-15
summary: 'An HTB machine — scanning with nmap to find FTP (21) running Microsoft ftpd with anonymous login and HTTP (80) running IIS 7.5 on a Windows 7 host, footprinting the web server with ZAP to identify the ASP.NET stack via X-Powered-By header, uploading a cmdasp.aspx webshell through anonymous FTP access, executing commands through the browser-accessible webshell, using PowerShell to discover the FTP root maps to C:\inetpub\ftproot, uploading nc.exe via FTP and setting up an impacket-smbserver for a reverse shell as iis apppool\web, running winPEAS to identify missing patches, and exploiting MS10-059 (Chimichurri) for privilege escalation to NT AUTHORITY\SYSTEM.'
role: 'pentest'
tags: ['nmap', 'ftp', 'iis', 'asp.net', 'webshell', 'anonymous-ftp', 'zap', 'netcat', 'impacket', 'winpeas', 'ms10-059', 'privilege-escalation', 'windows']
problem: 'Devel is an easy-rated Windows 7 machine with two open ports — FTP (21) running Microsoft ftpd with anonymous login enabled and write access to the IIS webroot, and HTTP (80) running Microsoft IIS 7.5 serving the default IIS7 welcome page. The FTP root directory maps directly to the IIS webroot (C:\inetpub\ftproot), meaning any file uploaded through anonymous FTP is immediately accessible via the web server. The IIS application pool runs as iis apppool\web, and the machine is missing the MS10-059 patch — a privilege escalation vulnerability in the Windows kernel (Chimichurri) that grants NT AUTHORITY\SYSTEM.'
action: 'Ran nmap with service version detection (-sV) and default scripts (-sC) to identify open ports — 21/tcp (FTP) running Microsoft ftpd with anonymous login allowed (FTP code 230) and 80/tcp (HTTP) running Microsoft IIS httpd 7.5 with the default IIS7 page. FTP directory listing showed aspnet_client, liststar.htm, and welcome.png. OS detection suggested Windows 7/Vista/2008/Phone 8.1/2012. Used ZAP to footprint the web server — identified ASP.NET from the X-Powered-By response header. Uploaded cmdasp.aspx webshell through anonymous FTP access. Accessed the webshell at http://10.129.228.10/cmdasp.aspx — confirmed command execution in the browser. Used PowerShell via the webshell to discover the FTP directory location (C:\inetpub\ftproot). Uploaded nc.exe via FTP and set up an impacket-smbserver to transfer it. Executed a reverse shell command through the webshell using nc.exe to obtain a shell as iis apppool\web. Ran winPEAS to enumerate the system for privilege escalation vectors — identified missing patches including MS10-059. Uploaded and executed the MS10-059 (Chimichurri) exploit to escalate privileges to NT AUTHORITY\SYSTEM. Retrieved both user and root flags.'
outcome: 'Gained SYSTEM-level access to the machine through a two-stage attack. The attack chain was nmap enumeration identifying anonymous FTP with IIS webroot access, ASP.NET webshell upload via FTP, command execution through the browser-accessible webshell, reverse shell as iis apppool\web via nc.exe, winPEAS enumeration identifying missing patches, and MS10-059 Chimichurri exploitation for privilege escalation to NT AUTHORITY\SYSTEM.'
draft: false
---
