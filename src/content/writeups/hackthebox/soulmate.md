...
problem: 'CrushFTP v11.3.0 on ftp.soulmate.htb is vulnerable to CVE-2025-31161 (auth bypass allowing arbitrary user creation). The admin panel lets you change any user''s password, and ben''s VFS maps webProd to the web root. An Erlang SSH daemon runs as root on port 2222 with hardcoded credentials in a start.escript.'
action: 'Nmap found SSH (22) and nginx (80) redirecting to soulmate.htb. Subdomain enumeration revealed ftp.soulmate.htb running CrushFTP v11.3.0. Exploited CVE-2025-31161 to create an admin user, changed ben''s password via User Manager, logged in as ben, and uploaded a PHP webshell to webProd/assets/. Caught a reverse shell as www-data. Found /usr/local/lib/erlang_login/start.escript with hardcoded credentials for ben (HouseH0ldings998) and an SSH daemon config on port 2222. SSH-ed to localhost:2222 as ben, landed in an Eshell running as root, and used os:cmd() for the root flag.'
outcome: 'Gained root. CrushFTP auth bypass to admin, webshell upload for www-data, hardcoded Erlang credentials for lateral movement, and root-level Erlang SSH daemon for privesc.'
draft: false
---
