...
difficulty: 'easy'
date: 2025-09-15
summary: 'An HTB machine — enumerating an Active Directory domain controller (support.htb) with anonymous SMB access to a support-tools share containing UserInfo.exe, extracting hardcoded LDAP credentials (ldap / nvEfEK16^1aM4$e7AclUf8x$tRWxPWO1%lmz) from the binary via strace, enumerating LDAP to find the support user''s password stored in the info attribute (Ironside47pleasure40Watchful), discovering that the Shared Support Accounts group (which support belongs to) has GenericAll-equivalent permissions on the DC$ object via ACL enumeration, creating a controlled computer account (EVIL1$), configuring Resource-Based Constrained Delegation on DC$, performing S4U impersonation to obtain an Administrator CIFS service ticket, and using it for command execution as SYSTEM to retrieve both flags.'
role: 'pentest'
...
problem: 'Support is an easy-rated Windows Active Directory machine running a domain controller (DC.support.htb) with standard AD services — DNS (53), Kerberos (88), LDAP (389/636), SMB (445), WinRM (5985), and Global Catalog (3268). The SMB support-tools share is anonymously readable and contains UserInfo.exe, a .NET binary with hardcoded LDAP bind credentials. The support domain user has its password stored in a cleartext LDAP attribute (info), belongs to the Shared Support Accounts group which has GenericAll-equivalent rights (WriteDACL, WriteOwner, WriteProperty) on the DC$ computer object, and the domain allows authenticated users to create machine accounts — enabling Resource-Based Constrained Delegation for domain compromise.'
action: 'Ran a port scan — identified 11 open ports including DNS (53), Kerberos (88), LDAP (389), SMB (445), and WinRM (5985) on DC.support.htb. Enumerated SMB shares anonymously — found support-tools share readable with 7 files including UserInfo.exe.zip. Downloaded and extracted UserInfo.exe — a .NET binary making LDAP queries. Extracted Unicode strings with monodis — found references to support\\ldap, LDAP://support.htb, and an encoded string. Ran UserInfo.exe under strace to capture the LDAP bind at runtime — the sendto() call revealed the plaintext password nvEfEK16^1aM4$e7AclUf8x$tRWxPWO1%lmz for the ldap user. Validated the credentials against LDAP — authenticated successfully. Enumerated domain users (20 accounts). Queried the support user''s attributes — found memberOf showing Shared Support Accounts and Remote Management Users groups, and the info attribute containing the plaintext password Ironside47pleasure40Watchful. Validated support credentials against LDAP. Performed RID brute-forcing via SMB to map SIDs — identified Shared Support Accounts at RID 1103. Queried the DC$ object ACL — found SID ending in -1103 (Shared Support Accounts) with WriteDACL, WriteOwner, ReadControl, Delete, CreateChild, DeleteChild, ListContents, Self, ReadProperty, WriteProperty, DeleteTree, ListObject, and ControlAccess. Created a controlled computer account EVIL1$ with password Password123. Set RBCD on DC$ allowing delegation from EVIL1$. Generated a krb5.conf for the domain. Requested a TGT for EVIL1$. Performed S4U2Self and S4U2Proxy to obtain a CIFS service ticket for Administrator to DC.support.htb. Tested command execution — whoami returned nt authority\\system. Obtained an interactive shell and retrieved both user.txt and root.txt flags. Dumped domain secrets including Administrator NT hash, machine account password, and DPAPI backup keys.'
outcome: 'Gained SYSTEM-level access to the domain controller. The attack chain was anonymous SMB access for binary extraction, LDAP credential recovery from UserInfo.exe via runtime interception, cleartext password in LDAP info attribute for the support user, ACL abuse through Shared Support Accounts group permissions on DC$, and Resource-Based Constrained Delegation with S4U impersonation for Administrator access.'
draft: false
---
