...
difficulty: 'easy'
date: 2026-02-01
summary: 'An HTB Sherlock from the Operation Blackout 2025 series — analysing Windows PowerShell event logs from a compromised workstation (DESKTOP-M3AKJSD) to reconstruct the virtualization detection techniques an attacker used for sandbox evasion, identifying that the attacker queried the Win32_ComputerSystem WMI class via Get-WmiObject to retrieve system model and manufacturer information for VM fingerprinting, discovering a thermal zone query SELECT * FROM MSAcpi_ThermalZoneTemperature used to detect physical hardware (virtual machines typically lack thermal sensors), finding that the attacker loaded Nishang Check-VM detection script from samratashok/nishang that fingerprints Hyper-V VMWare VirtualBox Virtual PC Xen and QEMU, tracing the script registry enumeration to HKLM:\SYSTEM\ControlSet001\Services where it checks for hypervisor-specific service names like vmicheartbeat vmicvss vmicshutdown vmicexchange for Hyper-V and vmdebug vmmouse VMTools VMMEMCTL for VMware, identifying VirtualBox detection through process comparison against vboxservice.exe and vboxtray.exe using Get-Process, and confirming the script detected both Hyper-V and VMWare platforms through Event ID 800 pipeline output logging the messages This is a Hyper-V machine and This is a VMWare machine.'
role: 'soc'
...
problem: 'Phantom Check is an easy-rated Hack The Box Sherlock from the Operation Blackout 2025 series presenting two Windows PowerShell event log files from a workstation where a threat actor carried out anti-virtualization checks to avoid detection in sandboxed environments. The investigation requires identifying the specific WMI classes and queries the attacker used to fingerprint the hardware, locating the VM detection script they loaded and its function name, tracing registry key enumeration paths used to identify hypervisor-specific services, determining which processes the script compares to detect VirtualBox, and confirming which virtualization platforms the script ultimately detected on the target system.'
action: 'Extracted the Phantom Check archive containing two event log files — Microsoft-Windows-Powershell.evtx (567 events) and Windows-Powershell-Operational.evtx (631 events), both from DESKTOP-M3AKJSD dated 09-04-2025. The Microsoft-Windows-Powershell log contained Event ID 800 entries recording Pipeline Execution Details with the full command lines of every PowerShell pipeline that ran, while the Windows-Powershell-Operational log contained Event ID 4104 entries recording scriptblock text — the actual source code of scripts that were executed. Opened the Microsoft-Windows-Powershell log in Event Viewer and used the Find function to search for Get-WmiObject, the standard PowerShell cmdlet for querying Windows Management Instrumentation. The first hit at 14:49:10 showed Event 800 with pipeline execution details for the command $Model = Get-WmiObject -Class Win32_ComputerSystem | select-object -expandproperty "Model", confirming the attacker used the Win32_ComputerSystem WMI class to retrieve the system model and manufacturer — virtual machines report distinctive model strings like "Virtual Machine" for Hyper-V or "VMware Virtual Platform" for VMware, making this a reliable fingerprinting technique. Continued searching Get-WmiObject hits and found a second entry at 14:50:12 showing Get-WmiObject -Query "SELECT * FROM MSAcpi_ThermalZoneTemperature" -ErrorAction SilentlyContinue — this WMI query attempts to read the ACPI thermal zone temperature of the machine, a clever detection technique because physical hardware has thermal sensors that report real temperature values while most virtual machines do not implement thermal zone emulation, so the query either fails or returns no data in a VM. The -ErrorAction SilentlyContinue flag suppresses errors when the query fails, allowing the script to silently determine it is virtualised without generating obvious error messages. Switched to the Windows-Powershell-Operational log and filtered for Event ID 4104 (scriptblock logging) to examine the actual scripts that ran. Found a Nishang framework script — the synopsis reads "Nishang script which detects whether it is in a known virtual machine" with a description stating "This script uses known parameters or fingerprints of Hyper-V, VMWare, Virtual PC, Virtual Box, Xen and QEMU for detecting the environment." The example usage shows PS > Check-VM, confirming the function name is Check-VM. The script links to the Nishang repository at github.com/samratashok/nishang by Nikhil SamratAshok Mittal, a well-known offensive PowerShell framework used in penetration testing. Examined subsequent Event 4104 entries to trace the Check-VM script source code. Found the Hyper-V detection block — the script uses $hyperv = Get-ChildItem HKLM:\SYSTEM\ControlSet001\Services to enumerate all services registered in the ControlSet001 services registry hive, then checks if any match vmicheartbeat, vmicvss, vmicshutdown, or vmicexchange — these are the Hyper-V Integration Services that are only present when the system is running as a Hyper-V guest. If any match, $hypervm is set to $true. Found the VirtualBox detection block marked with a comment #Virtual Box — the script uses $vb = Get-Process to enumerate all running processes, then checks if any match vboxservice.exe or vboxtray.exe. VBoxService.exe is the VirtualBox Guest Additions service that runs in the background handling host-guest communication, while VBoxTray.exe is the system tray utility that manages display resizing and clipboard sharing. Both processes are unique to VirtualBox guests and their presence is a definitive indicator. Found the VMware detection block — similar to Hyper-V, the script checks HKLM:\SYSTEM\ControlSet001\Services for service names matching vmdebug, vmmouse, VMTools, or VMMEMCTL, all of which are VMware-specific drivers and services installed by VMware Tools. Returned to the Microsoft-Windows-Powershell log and searched for "this is a" to find the script output — Event 800 at 14:50:57 showed the pipeline output with two ParameterBinding entries for Out-Default: value="This is a Hyper-V machine." and value="This is a VMWare machine.", confirming the Check-VM script detected both Hyper-V and VMware on the target system DESKTOP-M3AKJSD.'
outcome: 'Reconstructed the complete anti-virtualization reconnaissance from two PowerShell event log files: the attacker queried Win32_ComputerSystem via Get-WmiObject for hardware model fingerprinting, executed a thermal zone temperature query against MSAcpi_ThermalZoneTemperature to detect physical versus virtual hardware, loaded the Nishang Check-VM script that systematically fingerprints Hyper-V, VMware, VirtualBox, Virtual PC, Xen, and QEMU through registry service enumeration at HKLM:\SYSTEM\ControlSet001\Services and process comparison, and the script successfully detected both Hyper-V and VMware on DESKTOP-M3AKJSD. The investigation demonstrated how PowerShell event logging — particularly Event ID 800 for pipeline execution and Event ID 4104 for scriptblock text — provides complete visibility into attacker reconnaissance activities, and how WMI queries, registry checks, and process enumeration combine to form a comprehensive virtualization detection toolkit.'
draft: false
---
