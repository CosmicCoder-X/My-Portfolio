...
difficulty: 'medium'
date: 2025-08-29
summary: 'A combined walkthrough of two Active Directory rooms covering the full enumeration arc — from unauthenticated reconnaissance with Nmap, anonymous SMB access, and password policy extraction via enum4linux, through credential acquisition using CrackMapExec password spraying and AS-REP roasting with Hashcat, to authenticated domain enumeration with PowerShell AD cmdlets comparing Get-ADGroup against legacy net group output and querying AD objects by distinguished name and SID.'
role: 'pentest'
...
problem: 'A Windows Server 2019 domain controller for the tryhackme.loc domain is the target. The task spans two phases: first, enumerate the domain from an unauthenticated position — fingerprint services, extract data from anonymously accessible SMB shares, pull the domain password policy, and acquire valid credentials through password spraying and AS-REP roasting. Second, use those credentials to enumerate the domain from within — counting users, groups, and computers, mapping group memberships, comparing enumeration tools, and querying specific AD objects by distinguished name and SID.'
action: 'Ran a full Nmap service scan to fingerprint the domain controller and identify exposed services including Kerberos, LDAP, SMB, and RDP. Connected to the UserBackups SMB share via anonymous login to retrieve a flag. Used enum4linux to extract the domain password policy including minimum length, lockout thresholds, and complexity requirements. Performed a CrackMapExec password spray against the rduke account to recover valid credentials. Cracked AS-REP roasted hashes with Hashcat mode 18200 to obtain the asrepuser1 password. Authenticated via RDP and used PowerShell AD cmdlets to enumerate domain users, local users, groups, computers, and Domain Admin membership. Compared Get-ADGroup output against net group using Compare-Object to identify groups invisible to legacy tooling. Retrieved the Administrator object SID via Get-ADObject.'
outcome: 'Mapped the tryhackme.loc domain from zero access to full authenticated enumeration — identified the domain controller as Windows Server 2019 Datacenter (DC.tryhackme.loc), retrieved the SMB flag THM{88_SMB_88}, extracted a 7-character minimum password policy with 10-attempt lockout and 2-minute lockout duration, cracked rduke credentials (Password1!) via password spray and asrepuser1 credentials (qwerty123!) via AS-REP roasting, then enumerated 31 domain users, 5 local users, 54 domain groups (versus 21 visible to net group), 5 Domain Admins, 2 computers, 13 admin-related groups, and the Administrator SID S-1-5-21-4103247791-2828088783-3009141321-500.'
draft: false
---
