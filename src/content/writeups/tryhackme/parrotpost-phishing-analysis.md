...
difficulty: 'easy'
date: 2025-08-29
summary: 'A phishing email analysis exercise — inspecting email headers to identify spoofed sender details and originating IP (Latvia, emkei.lv), deobfuscating a multi-layered HTML attachment through base64 decoding, HTML entity decoding, CSS beautification, and JavaScript deobfuscation to reveal a credential harvesting page targeting ParrotPost Webmail, then submitting fake credentials to trace the exfiltration path to evilparrot.thm:8080/cred-capture.php and recovering stolen credentials from the server log.'
role: 'soc'
...
problem: 'Paul Feathers at flying-sec.thm received an email claiming to be from ParrotPost Webmail with an urgent account update notification. The email carries an HTML attachment that uses multiple layers of obfuscation — base64 encoding, HTML entity encoding, CSS minification, and JavaScript minification — to hide a credential harvesting page. The task is to analyse the email headers to identify the true origin, deobfuscate each layer of the attachment, understand the exfiltration mechanism, and trace the stolen credentials to the attacker server.'
action: 'Analysed the email headers using MXToolbox to identify the originating IP (109.205.120.0, Latvia) sent via emkei.lv (an anonymous email service), noted the spoofed Reply-To address (postparr0t.thm vs postparrot.thm), and extracted the custom header flag. Deobfuscated the HTML attachment through successive layers — decoded the base64 payload using CyberChef revealing the document.write(unescape(atob(b64))) execution chain, found a double-encoded base64 comment, decoded HTML entity obfuscation to recover the page title, beautified the minified JavaScript to identify the credential exfiltration URL (evilparrot.thm:8080/cred-capture.php) and the redirect mechanism (window.location.href). Rendered the phishing page in a browser, submitted fake credentials, captured the GET request in the Network tab showing credentials passed as URL parameters, and traced the stolen credential log at /creds.txt.'
outcome: 'Completed all tasks — identified Latvia as the sending country, no-reply@postparr0t.thm as the reply address, extracted THM{y0u_f0und_7h3_h34d3r} from the custom header, identified base64 encoding and atob() as the decoding function, recovered THM{d0ubl3_3nc0d3d} from the double-encoded comment, decoded the HTML entity layer revealing "ParrotPost Secure Webmail Login", identified CSS Beautify as the reverse of CSS Minify, traced the credential exfiltration to http://evilparrot.thm:8080/cred-capture.php with window.location.href as the redirect property, captured THM{c4p7ur3d_y0ur_cr3d5} from the response, identified /creds.txt as the credential log path, and recovered Chris Smith password FlyL1ke!A~Bird.'
draft: false
---
