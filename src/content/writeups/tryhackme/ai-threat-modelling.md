...
difficulty: 'medium'
date: 2025-08-29
summary: 'A defender-focused threat modelling exercise against MegaCorp AI chatbot architecture — identifying AI-specific assets and supply chain stages, adapting STRIDE categories for AI failure modes, enriching findings with MITRE ATLAS technique IDs, and mapping OWASP LLM Top 10 vulnerabilities to architectural components in a three-phase interactive assessment covering prompt injection at the web frontend, improper output handling at the API gateway, data poisoning in the training pipeline, supply chain vulnerabilities in the model registry, and vector embedding weaknesses in the RAG database.'
role: 'llm'
...
problem: 'MegaCorp has deployed an AI-powered chatbot with a RAG pipeline, a recommendation engine processing sensitive customer data, and an automated fraud detection system making real-time financial decisions. The CISO needs a comprehensive threat assessment before the quarterly board meeting — identifying which AI-specific assets expand the attack surface beyond traditional applications, mapping the AI data supply chain from collection through training to inference, adapting STRIDE for AI-specific failure modes, and using OWASP LLM Top 10 as the primary lens for mapping risks to architectural components with justified severity ratings.'
action: 'Worked through the full AI threat modelling methodology — identified AI-specific assets (embedding vectors, model registries, training data, system prompts), traced supply chain attack vectors to the data collection stage, adapted STRIDE categories for AI contexts where traditional mappings fall short (tampering insufficient for training data poisoning, elevation of privilege covering jailbreaking), mapped MITRE ATLAS technique IDs to attack patterns (AML.T0024 for model extraction, Morris II for self-replicating prompt injection worms), and completed the interactive OWASP LLM Top 10 threat assessment by selecting five vulnerabilities, mapping each to the correct architectural component across trust boundaries, justifying why each vulnerability applies at that point, and assigning appropriate criticality ratings.'
outcome: 'Completed the three-phase threat assessment with a perfect 75/75 score — correctly mapped LLM01 Prompt Injection to the Web Frontend (medium criticality, user input fields as the entry point before server-side validation), LLM05 Improper Output Handling to the API Gateway (medium criticality, gateway responsible for sanitising and validating model output before downstream propagation), LLM04 Data & Model Poisoning to the Training Pipeline (high criticality, poisoned training data creates manipulated behaviours and backdoors persisting after deployment), LLM03 Supply Chain Vulnerabilities to the Model Registry (high criticality, compromised models served directly to production without integrity verification), and LLM08 Vector & Embedding Weaknesses to the Vector Database (high criticality, adversarially crafted embeddings manipulating similarity search results to retrieve attacker-chosen content). Flag: THM{AI_THREAT_MODEL_COMPLETE}.'
draft: false
---
