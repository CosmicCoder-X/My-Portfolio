...
problem: "RAG-powered AI assistants retrieve context using semantic similarity with no concept of authorization. When public and confidential documents share a vector index, queries can pull sensitive content into the LLM context window. The task is to demonstrate these failure modes and apply preventive controls."
action: "Studied OWASP LLM02 threats, RAG retrieval mechanics, and vector database attack surfaces including embedding inversion and multi-tenant risks. Reviewed access control models and defensive safeguards. Completed the four-phase practical lab demonstrating broad retrieval disclosure, logging exposure, semantic collision between public and private documents, and remediation through metadata-based access controls."
outcome: "Completed all tasks — identified the key concepts (similarity retrieval, top-k parameters, cosine distance, embedding inversion, per-tenant isolation) and demonstrated broad retrieval and semantic collision as disclosure vectors with metadata filtering as the effective mitigation."
draft: false
---
