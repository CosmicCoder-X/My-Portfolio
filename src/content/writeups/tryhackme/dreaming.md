...
problem: "A Linux machine running Pluck 4.7.13 CMS behind a directory listing. The box requires chaining lateral movement across three user accounts, each protected by a different escalation mechanism."
action: "Discovered Pluck 4.7.13 via Gobuster, logged in with default credentials, and exploited a file upload vulnerability to get a web shell as www-data. Found credentials in Python scripts under /opt and SSH'd as lucien. Injected a command into the MySQL dreams table that getDreams.py executed via sudo, moving to death. Modified the system shutil.py imported by restore.py to chmod the final flag."
outcome: "Rooted the box through four privilege levels using file upload RCE, credential reuse, SQL-based command injection, and Python library hijacking. Retrieved all three flags including THM{DR34MS_5H4P3_TH3_W0RLD}."
draft: false
---
