...
difficulty: 'medium'
date: 2026-07-22
summary: 'A picoCTF Web Exploitation challenge — a Flask/Jinja2 announcement application vulnerable to Server-Side Template Injection where the input {{7*7}} rendered as 49 confirming SSTI, but unlike the predecessor SSTI1 the application enforced a blacklist filtering characters including dots, underscores, square brackets and the join keyword, which was bypassed by using Jinja2''s attr() filter combined with hexadecimal escape sequences (\x5f for underscores) to traverse the object hierarchy without triggering the filter — constructing the payload {{request|attr(''application'')|attr(''\x5f\x5fglobals\x5f\x5f'')|attr(''\x5f\x5fgetitem\x5f\x5f'')(''\x5f\x5fbuiltins\x5f\x5f'')|attr(''\x5f\x5fgetitem\x5f\x5f'')(''\x5f\x5fimport\x5f\x5f'')(''os'')|attr(''popen'')(''ls'')|attr(''read'')()}} which listed the server directory revealing __pycache__ app.py flag requirements.txt, then swapping the ls command for cat flag to read the flag file and retrieve picoCTF{sst1_f1lt3r_byp4ss_e39c23cf}.'
role: 'appsec'
tags: ['web-exploitation', 'ssti', 'jinja2', 'flask', 'python', 'filter-bypass', 'blacklist-bypass', 'template-injection', 'rce', 'picoctf']
problem: 'SSTI2 is a picoCTF Web Exploitation challenge that builds on the SSTI1 challenge by adding input filtering to a Flask/Jinja2 web application. The application still allows users to submit text that gets rendered as a template, but now blocks several characters and keywords commonly used in SSTI payloads — including dots, underscores, square brackets, and the join keyword. The objective is to bypass the blacklist and achieve remote code execution to read the flag file from the server.'
action: 'Accessed the challenge at http://shape-facility.picoctf.net:49891 which presented a simple web page titled "Home" with the text "I built a cool website that lets you announce whatever you want!*" and a text input field asking "What do you want to announce:" with an Ok button. The asterisk in the description was a hint that there were restrictions on what could be announced — the blacklist. Started by confirming the SSTI vulnerability was still present from the SSTI1 predecessor. Entered the classic Jinja2 test payload {{7*7}} into the input field and submitted it. The application navigated to /announce and rendered the output as 49 — the server had evaluated the arithmetic expression inside the template tags, confirming that user input was being processed through the Jinja2 template engine without proper escaping. With SSTI confirmed, the next step was understanding the restrictions. The SSTI1 challenge would have allowed direct Python object traversal using dot notation like {{request.application.__globals__.__builtins__.__import__("os").popen("ls").read()}}, but SSTI2 introduced a blacklist that blocked several critical characters: dots (preventing direct attribute access), underscores (blocking dunder attribute names like __globals__ and __builtins__), square brackets (preventing dictionary-style access), and the join keyword. These restrictions were specifically designed to break the standard SSTI payload chains that rely on traversing Python''s object hierarchy through dot notation and dunder attributes. The bypass strategy used Jinja2''s own template syntax against the filter. Instead of dot notation for attribute access, the attr() filter achieves the same thing — object.attribute becomes object|attr("attribute"). And instead of literal underscores in strings like __globals__, hexadecimal escape sequences work inside Python string literals — \x5f is the hex representation of the underscore character, so __globals__ becomes \x5f\x5fglobals\x5f\x5f when written as a string argument to attr(). The filter checks the raw input text for blocked characters, but \x5f only becomes an underscore after Python processes the string escape, by which point the filter has already passed the input through. With this approach, constructed the directory listing payload: {{request|attr("application")|attr("\x5f\x5fglobals\x5f\x5f")|attr("\x5f\x5fgetitem\x5f\x5f")("\x5f\x5fbuiltins\x5f\x5f")|attr("\x5f\x5fgetitem\x5f\x5f")("\x5f\x5fimport\x5f\x5f")("os")|attr("popen")("ls")|attr("read")()}}. Breaking down the chain: request accesses the Flask request object which is always available in the template context, |attr("application") gets the WSGI application object, |attr("\x5f\x5fglobals\x5f\x5f") accesses the global namespace (with hex-escaped underscores), |attr("\x5f\x5fgetitem\x5f\x5f")("\x5f\x5fbuiltins\x5f\x5f") retrieves the builtins module from globals, |attr("\x5f\x5fgetitem\x5f\x5f")("\x5f\x5fimport\x5f\x5f")("os") gets the __import__ function and imports the os module, |attr("popen")("ls") runs the ls command through os.popen, and |attr("read")() reads the command output. Submitting this payload returned the server''s directory listing rendered in large bold text on the /announce page: __pycache__ app.py flag requirements.txt. The presence of a file simply named flag confirmed where the challenge flag was stored. Swapped the ls command for cat flag in the same payload chain: {{request|attr("application")|attr("\x5f\x5fglobals\x5f\x5f")|attr("\x5f\x5fgetitem\x5f\x5f")("\x5f\x5fbuiltins\x5f\x5f")|attr("\x5f\x5fgetitem\x5f\x5f")("\x5f\x5fimport\x5f\x5f")("os")|attr("popen")("cat flag")|attr("read")()}}. The application rendered the flag file contents: picoCTF{sst1_f1lt3r_byp4ss_e39c23cf}.'
outcome: 'Retrieved the flag picoCTF{sst1_f1lt3r_byp4ss_e39c23cf} by bypassing the application''s blacklist filter using Jinja2''s attr() filter for attribute access and hexadecimal escape sequences (\x5f) to represent underscores. The challenge demonstrated a fundamental weakness of blacklist-based input sanitisation: it only blocks the specific character sequences the developer anticipated, but template engines and programming languages offer multiple ways to express the same operation. The attr() filter is functionally identical to dot notation, and \x5f is functionally identical to an underscore — the filter sees neither a dot nor an underscore in the raw input, yet the template engine resolves both to the same attribute traversal chain. The correct mitigation is not to add more entries to the blacklist but to avoid rendering user input as template code entirely, using proper output escaping or sandboxed rendering instead.'
draft: false
---
